"""Профили команды Narkologia (GOO NETWORK) — загрузка из API."""
...
async def fetch_aqua_team_profiles(
    *,
    ...
    if not team_key:
        raise AquaError("Не задан ключ команды Narkologia (NARKOLOGIA_TEAM_API_KEY)")
    ...
    if last_err and "404" in last_err:
        raise AquaError(
            ...
            "(код вида 7Fm70U0QUMU). Проверь также личный API key и NARKOLOGIA_TEAM_API_KEY на сервере."
        )
